    ...
    ref="leftTopView">
        ...
        <div id="leftTopView">
            <div class="view-box-con" ref="viewBoxCon">
                ...
                <div 
                v-for="(topicMsg,index) in topicMsgList" 
                :key="index" 
                >
                    <div 
                    class="systemTotalNum"
...
export default class SystemTopicProportion extends Vue{
    ...
    private systemOverlay:boolean = false
    ...
    async mounted() {
        ...
        let {count:countTotal}:topicMsg = this.topicMsgList.reduce(({count:prevcount}:topicMsg,{count:nextcount}:topicMsg) => {
            return {count:prevcount + nextcount,SystemName:""}
        });
        this.$nextTick(()=>{
            this.topicMsgList.forEach(({count,SystemName}:topicMsg,index:number) => {
                this.topicProportion(SystemName+"Total",{text:(count/countTotal*100).toFixed(2)+"%",subtext:`系统${SystemName}主题占比`,proportion:(count/countTotal).toFixed(4),color:this.colors[index]})
            });
            this.topicProportion("topicTotal",{text:countTotal+"",subtext:"主题数据总数",proportion:"1",color:"#195ba6"})
        })
        const el:any = this.$refs.leftTopView
