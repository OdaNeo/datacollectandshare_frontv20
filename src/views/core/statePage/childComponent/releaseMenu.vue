<template>
    ...
    :class="`${fristFlag?'bounceInUp':''} ${enterFlag?'enter':''}`"
            ...
            <div class="iconCon">
                ...
                >
                    ...
                    <v-list>
                        ...
                        :key="index"
                        @click="selectReleaseSystem(item)"
                        >
                ...
                >
                    ...
                    <v-list>
                        <v-list-item
                        v-for="num in parseInt(releaseTotal/10)+1"
                        :key="num"
                        @click="selectReleasePage(num)"
                        >
                            <v-list-item-title>{{`${(num-1)*10+1}~${num*10>releaseTotal?releaseTotal:num*10}`}}</v-list-item-title>
                        </v-list-item>
                    </v-list>
                </v-menu>
                <v-menu
...
export default class ReleaseMenu extends Vue{
    ...
    private releaseOverlay:boolean = false
    private releaseTopicExist:boolean = true
    private releaseStartTime:string = Moment(Moment().subtract(11, 'months').calendar(),"MM-DD-YYYY").format("YYYY-MM-DD")
    private releaseEndTime:string = Moment().format("YYYY-MM-DD")
    private releasePageNum:number = 1
    private releaseSystemId:number = 1
    private releaseSystemName:string = "ATS"
    private releaseTotal:number = 0
    ...
    private selectReleaseSystem(system:any){
    ...
    mounted() {
        ...
        this.getRelease({
            ...
            this.releaseOverlay = false
            this.drawRelease(result)
            const el:any = this.$refs.leftBtmView
