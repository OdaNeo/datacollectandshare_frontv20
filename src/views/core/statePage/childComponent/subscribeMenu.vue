<template>
    ...
    >
            ...
            <div class="iconCon">
                ...
                >
                    ...
                    <v-list>
                        ...
                        :key="index"
                        @click="selectSubscribeSystem(item)"
                        >
                ...
                >
                    ...
                    <v-list>
                        <v-list-item
                        v-for="num in parseInt(subscribeTotal/10)+1"
                        :key="num"
                        @click="selectSubscribePage(num)"
                        >
                            <v-list-item-title>{{`${(num-1)*10+1}~${num*10>subscribeTotal?subscribeTotal:num*10}`}}</v-list-item-title>
                        </v-list-item>
                    </v-list>
                </v-menu>
                <v-menu
        ...
        <div v-else >
            <p style="margin-top:150px">{{`主题${subscribeSystemName}在${subscribeStartTime}至${subscribeEndTime}没有发布主题`}}</p>
        </div> 
    </div>
</template>
<script lang="ts">
...
export default class SubscribeMenu extends Vue{
    @Prop() private systemItems!:any[]
    private fristFlag:boolean = true
    private enterFlag:boolean = false
    private subscribeOverlay:boolean = false
    private subscribeTopicExist:boolean = true
    private subscribeStartTime:string = Moment(Moment().subtract(11, 'months').calendar(),"MM-DD-YYYY").format("YYYY-MM-DD")
    private subscribeEndTime:string = Moment().format("YYYY-MM-DD")
    private subscribePageNum:number = 1
    private subscribeSystemId:number = 1
    private subscribeSystemName:string = "ATS"
    private subscribeTotal:number = 0
    private subscribeDate:string = new Date().toISOString().substr(0, 10)
    private subscribeTime:boolean = false
    ...
    private selectSubscribeSystem(system:any){
    ...
    mounted() {
        ...
        this.getSubscribe({
            ...
            this.subscribeOverlay = false
            this.drawSubscribe(result)
            const el:any = this.$refs.rightBtmView
