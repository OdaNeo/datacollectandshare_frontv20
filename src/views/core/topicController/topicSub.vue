    ...
    <div id="topicSub">
        ...
        >
            ...
            <template v-slot:operation="{item}">
                ...
                @click="item.status?'':subscribe(item)"
                >
                    {{item.status?"已订阅":"订阅"}}
                </v-btn>
            </template>
...
export default class TopicSub extends Vue{
    ...
    async searchMethod(bool:boolean,params:object){
        const {data}: returnDataType = bool?await this.h_request["httpGET"]<object>("GET_TOPICS_SELECTTOPICBYTIDTNAME",params):await this.h_request["httpGET"]<object>("GET_TOPICS_FIND_ALL_TOPIC",params)
    ...
    private async subscribe(item:any){
        const {success} = await this.h_request["httpPOST"]("POST_SUBMODERATIONS_INSERTSUBMODERATION",{"topicId":item.id})
        if(success){
            this.searchMethod(false,{
                pageSize:this.pageSize,
                pageNum:1
