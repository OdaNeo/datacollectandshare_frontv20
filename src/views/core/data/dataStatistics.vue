...
<script lang="ts">
    ...
    export default class DataStatistics extends Vue{
        ...
        private systemValue:string = ''
        private afterDate:string|null = ''
        private echartsProport:any = ''

        private noMsg:boolean = false
        private echartsType:string= 'Release'
        ...
        private showEchartsBox:boolean = false
        ...
        private topicListNumber:number = 1

        private releasePageNum:number = 1
        private selectMonth:Array<number> = [1,3,6,12]
        private currentSelectMonth:number = 1
        // 上个月
        private beforeMonth(){
            // 当前月份 - 选定月数
        ...
        private judgTopicMsg() {
            return this.echartsType == "Release"
                ? this.topicMsgRelease
                : this.topicMsgSubscribe;
        }
        ...
        private dataStructure(msg:any) {
            ...
            msg.forEach((element:any, index:number) => {
                let startTime = 0,
                    endTime = 0,
                    count = 0,
                    key:any = Object.keys(element);
                keys.push(key);
                element[key].forEach((item:any,index2:any) => {
                    if (!startTime || !endTime) {
                        if(!element[key][index2+1]){
                            let s = this.timeStamp(item.time);
                            let e = this.timeStamp(item.time)+12 * 60 * 60 * 1000;
                        ...
                        endTime = this.timeStamp(item.time);
                        count += parseInt(item.count);
                        if(!element[key][index2+1]||endTime+24*60*60*1000!=this.timeStamp(element[key][index2+1]['time'])){
                            res.push({
                                name: key[0],
        ...
        private yAxisShow(topicMsg:any) {
            let length = topicMsg.keys.length;
        ...
        private draw() {
            let options = {
                tooltip: {
                    formatter: (name:any) => {
                        let startTime = name.data.value[1],
                            endTime = name.data.value[2],
                            count = name.data.value[3],
                            topicName = name.data.name;
                        return (
                            this.beginDate +
                            "至" +
                            this.afterDate +
        ...
        private async getTopicList(){
            ...
            if(this.echartsType == "Release"){
                let Release = document.getElementById("canvasPackRelease");
                this.myChartRelease = (this as any).$echarts.init(Release);
            }else{
                let Subscribe = document.getElementById("canvasPackSubscribe");
        ...
        private async getData(){
            let doc = document.getElementById("canvasPackProportion");
