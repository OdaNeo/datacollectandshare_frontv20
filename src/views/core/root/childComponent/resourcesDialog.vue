<template>
    <v-row>
        ...
        v-show="formProvide.formObj.type"
        >
            <v-text-field
            single-line 
            outlined
            ...
            >
                <template v-slot:append>
                    <v-icon @click.stop="selectShow=!selectShow" :style="selectShow?'transform:rotate(180deg)':''" >mdi-chevron-down</v-icon>
                </template>
                <template v-slot:prepend>
                    <div class="text-label">
            ...
            <div v-show="selectShow" class="selectCon">
                ...
                <ul class="selectChildCon" v-show="selectChildShow">
                    <li 
                    v-for="(child,index) in selectChild.childrenList" 
                    :key="index"
                    @click="choiceLi(child,selectChild)"
                    >{{child.name}}</li>
                </ul>
            </div>
        </v-col>
...
export default class ResourcesDialog extends Vue{
    ...
    private actionSelect:number = -1
    private selectChild:Array<any> = []
    private selectShow:boolean = false
    private selectChildShow:boolean = false
    private selectModel = null

    private types:Array<any> = [{text:"菜单",value:"menu"},{text:"按钮",value:"button"}]

    private nameRules:Array<Function> = [
        (v:string) =>!!v||"请设置权限名称"
    ]

    private typeRules:Array<Function> = [
        (v:string) =>!!v||"请选择权限类型"
    ]

    private parentRules:Array<Function> = [
        (v:string) =>!!v||"请选择父节点"
    ]
