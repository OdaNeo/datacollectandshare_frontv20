<template>
  ...
  <div id="topicList">
    ...
    <!--  -->
    <h-dialog v-if="dialogFlag2" v-model="dialogFlag2" ref="HDialog">
      <data-structure-dialog slot="dialog-content" :rowObj="rowObj" v-if="dialogShow2 === 2"></data-structure-dialog>
      <create-topic-dialog
        slot="dialog-content"
        v-else-if="dialogShow2 === 1"
        ref="createTopicDialog"
        @create-protobuf-file="createProtobufFile"
      ></create-topic-dialog>
      <topic-ancillary-information-dialog
        slot="dialog-content"
        :otherObj="otherObj"
        v-else-if="dialogShow2 === 3"
      ></topic-ancillary-information-dialog>
    </h-dialog>
    <!-- 上传文件对话框 -->
...
export default class OnlineDataTopicList extends Vue {
  ...
  private dialogFlag2 = false // 弹窗展示
  private dialogShow = 0 // 展示哪个弹窗 1.rest 2.Protobuf 3.JSON

  private dialogShow2 = 0 // 展示哪个弹窗 1.是主题添加和修改弹窗 2.是详细信息弹窗 3.是附加信息弹窗
